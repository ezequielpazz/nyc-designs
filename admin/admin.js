...
/**
 * Manual trigger for the E-Pick shipment from the admin.
 * Hits /api/epick-crear-envio which is in sandbox mode until Sol gives us
 * credentials — at that point the same call goes live with no UI changes.
 */
async function createEpickShipment(orderId) {
    try {
        ...
        const resp = await fetch('/api/epick-crear-envio', {
            method: 'POST',
        ...
        showToast(`Envío creado: ${data.tracking_code}${data.sandbox ? ' (sandbox)' : ''}`, 'success');
        loadOrders(document.querySelector('.filter-tab.active')?.dataset.filter || 'todos');
    } catch (error) {
        console.error('❌ Error creating E-Pick shipment:', error);
        showToast('Error creando envío: ' + error.message, 'error');
    }
}
window.createEpickShipment = createEpickShipment;
